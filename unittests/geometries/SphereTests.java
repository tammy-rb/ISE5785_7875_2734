...
class SphereTests {
    ...
    void testFindIntersections() {
        ...
        assertEquals(expected01, result01, "Intersection points with the sphere are incorrect");
        ...
        assertNotNull(result02, "Ray intersects the sphere — result should not be null");
        assertEquals(1, result02.size(), "Wrong number of intersection points");
        assertEquals(expected02, result02, "Intersection point with the sphere is incorrect");

        // TC03: Ray starts outside the sphere and turns away from it
        Ray ray03 = new Ray(new Point(4, 1.5, 0), new Vector(1, 0.5, 0.5));
        assertNull(sphere.findIntersections(ray03), "Ray doesn't intersect — should return null");

        // TC04: Ray starts outside the sphere and misses it
        Ray ray04 = new Ray(new Point(4, 2, 2), new Vector(-0.5, 1, 0));
        assertNull(sphere.findIntersections(ray04), "Ray doesn't intersect — should return null");

        // =============== Boundary Values Tests ==================

        // TC101: Ray starts on the sphere and turns away from it
        Ray ray101 = new Ray(new Point(3, 0, 0), new Vector(1, 0.1, 0.1));
        assertNull(sphere.findIntersections(ray101), "Ray doesn't intersect — should return null");
        ...
        assertNotNull(result102, "Ray intersects the sphere — result should not be null");
        assertEquals(1, result102.size(), "Wrong number of intersection points");
        assertEquals(expected102, result102, "Intersection point with the sphere is incorrect");

        // TC103: Ray through center, starts on surface, turns away
        Ray ray103 = new Ray(new Point(2, 0, 1), new Vector(0, 0, 1));
        assertNull(sphere.findIntersections(ray103), "Ray doesn't intersect — should return null");
        ...
        assertNotNull(result104, "Ray intersects the sphere — result should not be null");
        assertEquals(1, result104.size(), "Wrong number of intersection points");
        assertEquals(expected104, result104, "Intersection point with the sphere is incorrect");
        ...
        assertNotNull(result105, "Ray intersects the sphere — result should not be null");
        assertEquals(1, result105.size(), "Wrong number of intersection points");
        assertEquals(expected105, result105, "Intersection point with the sphere is incorrect");

        // TC106: Ray through center, starts outside and goes away
        Ray ray106 = new Ray(new Point(4, 0, 0), new Vector(1, 0, 0));
        assertNull(sphere.findIntersections(ray106), "Ray doesn't intersect — should return null");
        ...
        assertNotNull(result107, "Ray intersects the sphere — result should not be null");
        assertEquals(2, result107.size(), "Wrong number of intersection points");
        assertEquals(expected107, result107, "Intersection points with the sphere are incorrect");
        ...
        assertNotNull(result108, "Ray intersects the sphere — result should not be null");
        assertEquals(1, result108.size(), "Wrong number of intersection points");
        assertEquals(expected108, result108, "Intersection point with the sphere is incorrect");

        // TC109: Ray is tangent, starts before tangent point
        Ray ray109 = new Ray(new Point(4, 1, 0), new Vector(-1, 0, 0));
        assertNull(sphere.findIntersections(ray109), "Ray doesn't intersect — should return null");

        // TC110: Ray is tangent, starts at tangent point
        Ray ray110 = new Ray(new Point(3, 0, 0), new Vector(0, 1, 0));
        assertNull(sphere.findIntersections(ray110), "Ray doesn't intersect — should return null");

        // TC111: Ray is tangent, starts after tangent point
        Ray ray111 = new Ray(new Point(3, 1, 0), new Vector(0, 1, 0));
        assertNull(sphere.findIntersections(ray111), "Ray doesn't intersect — should return null");
        ...
        assertNotNull(result112, "Ray intersects the sphere — result should not be null");
        assertEquals(1, result112.size(), "Wrong number of intersection points");
        assertEquals(expected112, result112, "Intersection point with the sphere is incorrect");

        // TC113: Ray parallel to tangent, starts outside sphere
        Ray ray113 = new Ray(new Point(4, 1.1, 0), new Vector(1, 0, 0));
        assertNull(sphere.findIntersections(ray113), "Ray doesn't intersect — should return null");
    }
}